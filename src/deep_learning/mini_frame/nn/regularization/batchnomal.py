...
class BatchNormalization(Model):
    ...
    def forward(self, X):
        ...
        if self.training:
            ...
            moving_mean = (
                np.mean(self.X, axis=0, keepdims=True) * 0.1 + self.moving_mean * 0.9
            )
            moving_std = (
                np.sqrt(np.var(self.X, axis=0, keepdims=True) + self.eps) * 0.1
                + self.moving_std * 0.9
            )
            return (X - moving_mean) / moving_std * self.gamma + self.beta
